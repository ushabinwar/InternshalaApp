...
import { addInternshipPost, addJobPost, fetchInternshipDetails, loadEmployeDetails } from '../../store/userAction'
import { toast } from 'react-toastify'
import { useParams } from 'react-router-dom'
import { updateInternshipDetails } from '../../store/userSlice'


const EditInterrnshipPost = ({onClose}) => {
  console.log(onClose)
  ...
  useEffect(() => {
    if (internshipDetails) {
      setformData({
        profile: internshipDetails.internship.profile || '',
        skills: internshipDetails.internship.skill || '',
        internshiptype: internshipDetails.internship.internshiptype || '',
        openings: internshipDetails.internship.openings || 0,
        from: internshipDetails.internship.from || '',
        to: internshipDetails.internship.to || '',
        duration: internshipDetails.internship.duration || '',
        responsibility: internshipDetails.internship.responsibility || '',
        perks: internshipDetails.internship.perks || '',
        // stipendStatus: internshipDetails.internship.stipend.status || '',
        // stipendAmount: internshipDetails.internship.stipend.amount || 0,
        assesments: internshipDetails.internship.assesments || '',
        location: internshipDetails.internship.location || ''
      });
    }
  }, [internshipDetails]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    dispatch(updateInternshipDetails(internshipId, formData));
    onClose();
    toast.success('Internship Post Updated');
  };
...
export default EditInterrnshipPost;